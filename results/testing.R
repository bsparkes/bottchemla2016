...
p.perc <- ddply(p,.(BetCat, primeStrengthText),summarise,FIX.PROP = sum(responseChoice)/length(responseChoice))
p.perc


ggplot(p.perc,aes(x=factor(BetCat),y=FIX.PROP, fill=primeStrengthText, label=paste(round(FIX.PROP*100), "%"))) +
...
summary(psswp)
psswp.perc <- ddply(psswp,.(primeStrengthText),summarise,FIX.PROP = sum(responseChoice)/length(responseChoice))

ggplot(psswp.perc,aes(x=factor(primeStrengthText),y=FIX.PROP,label=paste(round(FIX.PROP*100), "%")),fill=primeStrength) +
