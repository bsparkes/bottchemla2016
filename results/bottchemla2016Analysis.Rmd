...
contrasts(rp$WithBet) <- contr.sum(2)
contrasts(rp$primeStrengthText) <- contr.sum(2)
# contrasts(rpS$WithBet)
# contrasts(rpS$primeStrengthText)
...
# contrasts(rpS$WithBet)
# contrasts(rpS$primeStrengthText)
contrasts(rpS$WithBet) <- contr.treatment(2)
contrasts(rpS$primeStrengthText) <- contr.sum(2)
contrasts(rpS$WithBet)
contrasts(rpS$primeStrengthText)
contrasts(rpS$responseChoice)

rp.simpleModel <- glmer(responseChoice ~ primeStrengthText*WithBet + (1 + primeStrengthText*WithBet | workerid), data=rpS, family="binomial", contrasts = list(WithBet = "contr.treatment", primeStrengthText = "contr.sum"))
...
contrasts(wrp$primeStrengthText) <- contr.sum(2)
# contrasts(wrp$WithCat)
# contrasts(wrp$responseChoice)
...
contrasts(brp$BetCat) <- contr.treatment(6)
contrasts(brp$primeStrengthText) <- contr.sum(2)
# contrasts(brp$BetCat)
